  ...
  const stats = useMemo(() => {
    if (!entries)
      return {
        ...
        longestEntry: 0,
        shortestEntry: 0,
        moodDistribution: [] as { mood: string; count: number; color: string; label: string }[],
    ...
    // Word statistics
    const wordCounts = filteredEntries.map(e => e.content.trim().split(/\s+/).length);
    ...
    const growthRate = lastMonth > 0 ? Math.round(((currentMonth - lastMonth) / lastMonth) * 100) : 0;
    ...
    // Activity by day
    const activityByDay = Object.entries(dayOfWeekCounts).map(([day, count]) => ({
      day,
      count,
    }));
  ...
  if (!entries) {
    return (
      <View className="flex-1 justify-center items-center bg-gray-50">
        ...
        <Text className="text-gray-500 mt-4 font-medium">Loading insights...</Text>
      </View>
    );
  }
        ...
        <View className="flex-row gap-2 justify-center">
          {(["week", "month", "year", "all"] as const).map((period) => (
            ...
            >
              ...
              >
                {period === "week" ? "7 Days" : period === "month" ? "Month" : period === "year" ? "Year" : "All Time"}
              </Text>
            </Pressable>
          ))}
        </View>
      ...
      >
        ...
        {stats.total > 0 && (
          <View className="mb-6">
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} className="flex-row gap-3">
              {stats.streak >= 7 && (
                <View className="bg-gradient-to-r from-orange-400 to-red-500 p-4 rounded-2xl items-center" style={{ width: 120 }}>
                  <Text className="text-3xl mb-1">🔥</Text>
                  <Text className="text-white text-xs font-bold">Week Streak</Text>
                </View>
              )}
              {stats.total >= 10 && (
                <View className="bg-gradient-to-r from-blue-400 to-blue-600 p-4 rounded-2xl items-center" style={{ width: 120 }}>
                  <Text className="text-3xl mb-1">📚</Text>
                  <Text className="text-white text-xs font-bold">10+ Entries</Text>
                </View>
              )}
              {stats.totalWords >= 1000 && (
                <View className="bg-gradient-to-r from-purple-400 to-purple-600 p-4 rounded-2xl items-center" style={{ width: 120 }}>
                  <Text className="text-3xl mb-1">✍️</Text>
                  <Text className="text-white text-xs font-bold">1K+ Words</Text>
                </View>
              )}
              {stats.longestStreak >= 30 && (
                <View className="bg-gradient-to-r from-green-400 to-green-600 p-4 rounded-2xl items-center" style={{ width: 120 }}>
                  <Text className="text-3xl mb-1">🌟</Text>
                  <Text className="text-white text-xs font-bold">30-Day Hero</Text>
                </View>
              )}
            </ScrollView>
          </View>
          ...
          <View className="flex-row flex-wrap gap-3">
            ...
            >
              <View className="flex-row items-center mb-2">
                ...
                <Text className="text-gray-500 text-xs ml-2">Total Entries</Text>
              </View>
            ...
            >
              ...
              <Text className={`text-3xl font-bold ${stats.growthRate >= 0 ? 'text-green-600' : 'text-red-600'}`}>
        ...
        <View className="mb-6">
          ...
          >
            <View className="flex-row justify-between items-center mb-3 pb-3 border-b border-gray-100">
              ...
              <Text className="text-gray-900 text-lg font-bold">{stats.avgWordsPerEntry} words</Text>
            </View>
            <View className="flex-row justify-between items-center mb-3 pb-3 border-b border-gray-100">
              <Text className="text-gray-600 text-sm">Longest Entry</Text>
              <Text className="text-gray-900 text-lg font-bold">{stats.longestEntry} words</Text>
            </View>
            <View className="flex-row justify-between items-center">
              <Text className="text-gray-600 text-sm">Shortest Entry</Text>
              <Text className="text-gray-900 text-lg font-bold">{stats.shortestEntry} words</Text>
            </View>
          </View>
        </View>
          ...
          >
            <View className="flex-row items-center justify-between mb-4">
              <View className="flex-1">
                <Text className="text-gray-500 text-xs mb-1">Current Streak</Text>
              ...
              <View className="flex-1 items-end">
                <Text className="text-gray-500 text-xs mb-1">Longest Streak</Text>
        ...
        {stats.mostProductiveDay && (
          <View className="mb-6">
            ...
            >
              <Text className="text-gray-600 text-sm mb-2">Most Productive Day</Text>
                    ...
                    <View key={index} className="mb-3">
                      <View className="flex-row items-center justify-between mb-1">
                        ...
                        <Text className="text-gray-900 text-sm font-bold">
                          {item.count} ({Math.round((item.count / stats.total) * 100)}%)
                        </Text>
                      </View>
              ...
              {stats.mostUsedMood && (
                <View className="bg-green-50 p-3 rounded-xl border border-green-200">
                  <Text className="text-green-700 text-xs font-semibold text-center">
                    ...
                    {MOOD_EMOJIS.find(m => m.emoji === stats.mostUsedMood)?.label}
                  </Text>
                </View>
              )}
            </View>
                ...
                {stats.activityByDay.map((item, index) => {
                  ...
                  return (
                    <View key={index} className="items-center flex-1">
                      <Text className={`text-xs font-bold mb-1 ${isHighest ? 'text-green-600' : 'text-gray-900'}`}>
                        {item.count}
                      </Text>
                      <View
                      ...
                      />
                      <Text className={`text-xs font-semibold mt-2 ${isHighest ? 'text-green-600' : 'text-gray-600'}`}>
                        {item.day}
                      </Text>
                    </View>
                  );
        ...
        {stats.total === 0 && (
          <View className="bg-white p-8 rounded-2xl border border-gray-100 items-center">
            ...
            >
              <Text className="text-white font-bold text-sm">Create First Entry</Text>
            </TouchableOpacity>
          </View>
        )}
      </ScrollView>
